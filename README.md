# RescueMaze
敵AIの視野を避けながら、迷路内にいる救出対象をゴールまで導く一人称視点のアクションゲームです。

## 概要
プレイヤーは敵が徘徊する迷路を探索し、迷っている救出対象をすべて見つけてゴールまで導きます。

敵には視野角と視野距離が設定されており、プレイヤーを発見すると追跡を開始します。また、敵が救出対象に接触すると、救出対象は初期位置へ戻されます。

プレイヤーは一人称視点と上空カメラを切り替えながら、敵の視野を避けて迷路を攻略します。

<p align="center">
  <img src="Images/game.png" alt="RescueMazeGameplay" width="640">
</p>


以下のgifは、クリアアニメーションの様子です。
ゲーム全体のサイクルが完結するように作り込みました。

<p align="center">
  <img src="Images/clear.gif" alt="RescueMazeGameplay" width="640">
</p>


## 開発環境
- Windows 11
- Unity 6.3
- C#
- Blender（タイトル画面制作）

## 音楽・効果音
- 魔王魂　様
- 無料効果音で遊ぼう！ 様
- On-Jin ～音人～ 様

## 操作方法
| 操作 | 内容 |
|------|------|
| WASD | 移動 |
| マウス | 一人称視点の操作 |
| Tab | 一人称視点／上空カメラ切り替え |
| R | ステージリスタート／コンテニュー |
| Space | ステージクリア後に次のステージへ |
| Esc | タイトル画面でゲーム終了 |

## 実装した機能
- 敵AI（視野判定・追跡）
- 一人称視点／上空カメラ切り替え
- 救出対象の追従システム
- 説明用UI
- タイトル画面
  
### 追加した機能 Ver. 1.1.0
- 難易度「ノーマル」を追加
- 敵AIの種類追加（その場で回転）
- 難易度「ノーマル」のみに、タイマー機能追加
- 時間切れによるゲームオーバー処理を追加
- 風景アセットを用いて演出を強化

### 変更した機能 Ver. 1.2.0
- ルールに変化が生じるため、難易度「ノーマル」のタイマー機能を削除
- 敵スクリプトのクラス化
- 救出対象と敵をプレハブ化

### 変更した機能 Ver. 1.3.0
- 難易度「ハード」を追加
- サウンド調節機能を追加

## 工夫した点
- 敵ごとに視野角・視野距離を設定し、壁を挟まない場合のみプレイヤーを検知する視野判定を実装した。
- カメラの向きを基準としたプレイヤー操作を実装した。
- 救出対象がプレイヤーと重ならないよう追従位置を調整し、複数の救出対象にも対応できるようオフセットを設定した。
- タイトル画面では背景のアニメーションと環境音を組み合わせ、ゲームの雰囲気を演出した。
- 難易度選択機能を実装することで、プレイヤーのゲームスキルに合わせてプレイできるようにした。
- 操作方法をUIとして画面に表示することで、プレイヤーが快適にプレイできるようにした。
- サウンド調節機能も追加し、より快適にプレイできるようにした。

## 今後の改善点
- 敵の巡回パターンを増やし、ゲーム性を向上させる。
- 敵とプレイヤーの当たり判定を調整し、操作性を改善する。
- 救出対象の衝突判定を改善し、壁をすり抜けないようにする。

## 動画
Ver. 1.0
[https://youtu.be/VWtFZuNf9bs](https://youtu.be/VWtFZuNf9bs)

Ver. 1.1.0
[https://youtu.be/sb0J-VwltkU](https://youtu.be/6euHPgr9SKY)

## 使用したアセット
[AllSky Free - 10 Sky / Skybox Set](https://assetstore.unity.com/packages/2d/textures-materials/sky/allsky-free-10-sky-skybox-set-146014)

